                                         ...
                                         const std::string &package_name)
{
    ...
    if (curl)
    {
        ...
        RCLCPP_INFO(get_logger(), "Download Package finished: %s with result %d ",
                    url_to_file.c_str(), res);
        fclose(fp);
        return (unpack_package(target_file, target_path, package_name));
    }
}
                                         ...
                                         const std::string &package_name)
{
    ...
    if (curl)
    {
        ...
        RCLCPP_INFO(get_logger(), "Download Package finished: %s with result %d ",
                    url_to_file.c_str(), res);
        fclose(fp);
        return (unpack_message(target_file, target_path, package_name));
    }
}
                                       ...
                                       const std::string &package_name)
{
    ...
    else
    {
        for (;;)
        {
            ...
            {
                RCLCPP_ERROR(get_logger(), "Read data to target failed %s",
            ...
            if (entry_path.compare(lib_name) == 0)
            {
                ...
                else if (archive_entry_size(entry) > 0)
                {
                    ...
                    if (r < ARCHIVE_OK)
                    {
                        RCLCPP_ERROR(get_logger(), "Writing data to target failed %s",
                                     archive_error_string(ext));
                        ret_val = false;
                        break;
                    }
                ...
                if (r < ARCHIVE_OK)
                {
                    RCLCPP_ERROR(get_logger(), "Writing final data to target failed %s",
                                 archive_error_string(ext));
                    ret_val = false;
                    break;
                }
