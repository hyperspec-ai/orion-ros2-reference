...
bool LifecycleManager::change_state(std::uint8_t transition,
...
void LifecycleManager::add_node(NodeCfg nfc)
{
    ...
    RCLCPP_INFO(get_logger(),
                "Creating for Node %s lifecycle client, with topic %s and %s.",
                nfc.m_name.c_str(),
                cs_topic.c_str(),
                gs_topic.c_str());
...
bool LifecycleManager::check_state_all_nodes()
{
    for(NodeCfg node : m_configured_nodes)
    {
        ...
        {
            RCLCPP_ERROR(
                get_logger(), "Failed to get current state for node %s", node.m_name.c_str());
        }
    }
    return (true);
}
