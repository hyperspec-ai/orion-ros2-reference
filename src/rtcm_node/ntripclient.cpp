...
#include "socket.hpp"
namespace ntrip {
NtripClient::NtripClient(const std::string& host,
                         uint16_t port,
...
void NtripClient::worker_thread()
{
    while(m_running)
    {
        ...
        {
            RCLCPP_INFO(
                rclcpp::get_logger("NTRIP"), "Address Resolution failed %s", m_host.c_str());
            sleep(5);
        }
        else
        {
            ...
            if(::connect(socket, info->ai_addr, info->ai_addrlen) < 0)
            {
                freeaddrinfo(info);
                RCLCPP_INFO(
                    rclcpp::get_logger("NTRIP"), "Connect failed %s:%d", m_host.c_str(), m_port);
                sleep(5);
            }
            else
            {
                ...
                if(recv_state == RECV_OK)
                {
                    if(resp.status == Response::STATUS_OK)
                    {
                        ...
                        while(m_running)
                        {
                            ...
                            {
                                RCLCPP_INFO(rclcpp::get_logger("NTRIP"),
                                            "Connection dropped %s:%d with Code %d",
                                            m_host.c_str(),
                                            m_port,
                                            resp.status);
                                break;
                    ...
                    {
                        RCLCPP_INFO(rclcpp::get_logger("NTRIP"),
                                    "Connection failed %s:%d with Code %d",
                                    m_host.c_str(),
                                    m_port,
                                    resp.status);
                        sleep(10);
...
uint8_t NtripClient::write_to_socket(Socket& socket, std::string request_data)
{
    ...
    while(remaining > 0)
    {
        ...
        {
            RCLCPP_INFO(
                rclcpp::get_logger("NTRIP"), "Failed to send data %s:%d", m_host.c_str(), m_port);
            ret_val = false;
            break;
        }
        remaining -= size;
...
Response NtripClient::read_from_socket(Socket& socket,
                                       recv_success_t* state,
                                       recv_state_t recv_state)
{
    ...
    *state = RECV_OK;
    ...
    for(;;)
    {
        ...
        {
            RCLCPP_INFO(
                rclcpp::get_logger("NTRIP"), "Connection lost %s:%d", m_host.c_str(), m_port);
        ...
        else
        {
            ...
            if(!parsed_headers)
            {
                for(;;)
                {
                    const auto i = std::search(response_data.begin(),
                                               response_data.end(),
                                               std::begin(clrf),
                                               std::end(clrf));
                    ...
                    else // headers
                    {
                        ...
                        if(pos != std::string::npos)
                        {
                            ...
                            headerValue.erase(
                                headerValue.begin(),
                                std::find_if(headerValue.begin(), headerValue.end(), [](int c) {
                                    return !std::isspace(c);
                                }));

                            // rtrim
                            headerValue.erase(std::find_if(headerValue.rbegin(),
                                                           headerValue.rend(),
            ...
            if(parsed_headers)
            {
                ...
                else
                {
                    response.body.insert(
                        response.body.end(), response_data.begin(), response_data.end());
                    response_data.clear();
